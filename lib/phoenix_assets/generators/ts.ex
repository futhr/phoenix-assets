defmodule PhoenixAssets.Generators.TS do
  ...
  def type_import(types) do
    names = Enum.map(types, &validated_type_name!/1)
    ~s|import type { #{Enum.join(names, ", ")} } from "$phoenix/types"\n|
  end
  ...
  def url_template(path) do
    ...
    {segments, _} =
      ...
      |> Enum.map_reduce(names, fn
        ...
        segment, rest ->
          {escape_template_segment(segment), rest}
      end)
  ...
  def type_name(type) when is_binary(type), do: validated_type_name!(type)
  ...
  def type_name(type) when is_atom(type) do
    ...
    validated_type_name!(name)
  end

  def type_name(type), do: raise(ArgumentError, "invalid TypeScript type name: #{inspect(type)}")

  defp validated_type_name!(name) do
    if Regex.match?(@identifier, name) and name not in @reserved do
      name
    else
      raise ArgumentError, "invalid TypeScript type name: #{inspect(name)}"
    end
  end

  defp escape_template_segment(segment) do
    segment
    |> String.replace("\\", "\\\\")
    |> String.replace("`", "\\`")
    |> String.replace("${", "\\${")
