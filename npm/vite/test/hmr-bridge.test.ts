...
it("invalidates both locale aliases and tears down pending work and listeners", () => {
  const plugin = devClientPlugin({ generatedDir: "src/generated" })
  hook(plugin.configResolved).call({}, { root: "/app", command: "serve" })
  const server = fakeServer()
  const off = vi.fn()
  Object.assign(server.watcher, { off })
  const lookup = vi.spyOn(server.moduleGraph, "getModuleById")
  hook(plugin.configureServer).call({}, server)
  server.handlers.change(path.resolve("/app/src/generated/locales.ts"))
  vi.runAllTimers()
  expect(lookup).toHaveBeenCalledWith("\0phoenix-assets:localize")
  expect(lookup).toHaveBeenCalledWith("\0phoenix-assets:locales")
  server.handlers.change(path.resolve("/app/src/generated/env.ts"))
  hook(plugin.closeBundle).call({})
  vi.runAllTimers()
  expect(server.sent).toHaveLength(1)
  expect(off.mock.calls.map(([event]) => event)).toEqual(["change", "add", "unlink"])
})
