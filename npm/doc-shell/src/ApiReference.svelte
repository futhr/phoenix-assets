...
const { spec, baseUrl = "", allowedOrigins = [] }: Props = $props()
...
<div class="api-reference"><nav>{#if spec.info}<h2>{spec.info.title}</h2><small>v{spec.info.version}</small>{/if}{#each Object.entries(groups) as [tag, entries] (tag)}<h3>{tag}</h3>{#each entries ?? [] as entry (entry.id)}<button type="button" onclick={() => (selected = entry)}><strong>{entry.method}</strong> {entry.path}</button>{/each}{/each}</nav><main>{#if selected}<OperationCard entry={selected} /><TryIt entry={selected} {baseUrl} {allowedOrigins} />{/if}</main></div>
